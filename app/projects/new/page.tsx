...
export default function NewProject() {
  ...
  const handleEmployeeKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      e.preventDefault(); // Force stops the form from submitting
      addEmployee();
    }
  };
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-lg border border-green-100 animate-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
        ...
        <div className="bg-green-50/30 p-5 md:p-6 rounded-2xl md:rounded-[2rem] border border-green-100">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <input value={empName} onChange={e => setEmpName(e.target.value)} onKeyDown={handleEmployeeKeyDown} className="flex-1 border-2 border-green-100 focus:border-green-600 focus:ring-0 rounded-xl p-4 bg-white outline-none transition-colors font-semibold" placeholder="Type employee name..." />
            <button type="button" onClick={addEmployee} className="bg-green-700 text-white px-6 py-4 sm:py-0 rounded-xl font-bold hover:bg-green-800 transition-colors shadow-md flex items-center justify-center gap-2 active:scale-95">
              <Plus size={20}/> Add
            </button>
          </div>
        ...
        <div className="bg-green-50/30 p-5 md:p-6 rounded-2xl md:rounded-[2rem] border border-green-100">
          ...
          <button type="button" onClick={() => setModalOpen(true)} className="w-full py-5 border-2 border-dashed border-green-400 rounded-xl text-green-700 font-bold hover:bg-green-100 hover:border-green-500 transition-all flex justify-center items-center gap-2 bg-white shadow-sm active:scale-95">
          ...
          <div className="mt-4 space-y-3">
            {equipments.map((eq, i) => (
              <div key={i} className="flex justify-between items-center bg-white border-2 border-green-100 p-4 rounded-xl shadow-sm">
                ...
                <button type="button" onClick={() => setEquipments(equipments.filter((_, idx) => idx !== i))} className="p-2 hover:bg-red-50 rounded-lg transition-colors group">
        ...
        <button type="submit" disabled={isSubmitting || !name} className="w-full bg-green-800 text-white py-5 rounded-2xl font-black text-lg md:text-xl hover:bg-green-900 shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-4 active:scale-95">
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-green-950/60 z-50 flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white p-6 md:p-8 rounded-[2rem] w-full max-w-lg shadow-2xl animate-in">
            <div className="flex justify-between items-center mb-6 border-b-2 border-green-50 pb-4">
              ...
              <button type="button" onClick={() => setModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><X size={24} className="text-gray-500" /></button>
            </div>
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 custom-scrollbar pr-2">
              {searchResults.map(res => (
                <div key={res.id} onClick={() => addEquipment(res)} className="p-4 bg-white border-2 border-gray-100 rounded-xl cursor-pointer hover:border-green-500 hover:bg-green-50 transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 group">
              ...
              {search.length > 1 && searchResults.length === 0 && (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-500 font-medium">No main equipment found.</p>
                </div>
              )}
            </div>
          </div>
