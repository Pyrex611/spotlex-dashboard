    ...
    <div className="max-w-3xl mx-auto bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-lg border border-green-100 animate-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
        <div>
          ...
          <input required value={name} onChange={e => setName(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter') e.preventDefault() }} className="w-full border-2 border-green-100 focus:border-green-600 focus:ring-0 rounded-xl md:rounded-2xl p-4 bg-green-50/50 outline-none transition-colors text-lg font-bold text-green-950" placeholder="e.g. Lagos Lagoon Cleanup" />
        </div>
        ...
        <div className="bg-green-50/30 p-5 md:p-6 rounded-2xl md:rounded-[2rem] border border-green-100">
          ...
          <div className="flex flex-row gap-2 md:gap-3 mb-4">
            ...
            {/* The onPointerDown intercepts the touch BEFORE the mobile keyboard hides */}
            <button 
              type="button" 
              onPointerDown={(e) => { e.preventDefault(); addEmployee(); }}
              onClick={addEmployee}
              className="bg-green-700 text-white px-5 md:px-6 rounded-xl font-bold hover:bg-green-800 transition-colors shadow-md flex flex-shrink-0 items-center justify-center gap-1.5 active:scale-95 touch-manipulation"
            >
              <Plus size={20}/> Add
            </button>
          </div>
        ...
        <div className="bg-green-50/30 p-5 md:p-6 rounded-2xl md:rounded-[2rem] border border-green-100">
          ...
          <button type="button" onClick={() => setModalOpen(true)} className="w-full py-5 border-2 border-dashed border-green-400 rounded-xl text-green-700 font-bold hover:bg-green-100 hover:border-green-500 transition-all flex justify-center items-center gap-2 bg-white shadow-sm active:scale-95 touch-manipulation">
          ...
          <div className="mt-4 space-y-3">
            {equipments.map((eq, i) => (
              <div key={i} className="flex justify-between items-center bg-white border-2 border-green-100 p-4 rounded-xl shadow-sm">
                ...
                <button type="button" onClick={() => setEquipments(equipments.filter((_, idx) => idx !== i))} className="p-2 hover:bg-red-50 rounded-lg transition-colors group touch-manipulation">
        ...
        <button type="submit" disabled={isSubmitting || !name} className="w-full bg-green-800 text-white py-5 rounded-2xl font-black text-lg md:text-xl hover:bg-green-900 shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-4 active:scale-95 touch-manipulation">
        ...
        <div className="fixed inset-0 bg-green-950/60 z-[100] flex items-center justify-center p-4 backdrop-blur-md">
          <div className="bg-white p-6 md:p-8 rounded-[2rem] w-full max-w-lg shadow-2xl animate-in">
            <div className="flex justify-between items-center mb-6 border-b-2 border-green-50 pb-4">
              ...
              <button type="button" onClick={() => setModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors touch-manipulation"><X size={24} className="text-gray-500" /></button>
            </div>
            ...
            <div className="max-h-60 overflow-y-auto space-y-2 custom-scrollbar pr-2">
              {searchResults.map(res => (
                <div key={res.id} 
                     onPointerDown={(e) => { e.preventDefault(); addEquipment(res); }}
                     onClick={() => addEquipment(res)} 
                     className="p-4 bg-white border-2 border-gray-100 rounded-xl cursor-pointer hover:border-green-500 hover:bg-green-50 transition-all flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 group touch-manipulation">
                  <span className="font-bold text-gray-800 group-hover:text-green-900 text-sm md:text-base pointer-events-none">{res.name}</span>
                  <span className="text-xs font-mono bg-gray-100 text-gray-600 px-3 py-1.5 rounded-lg group-hover:bg-green-200 group-hover:text-green-800 transition-colors font-bold w-full sm:w-auto text-center pointer-events-none">{res.code}</span>
                </div>
              ))}
